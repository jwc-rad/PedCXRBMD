...
class SamplerDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        _sampler = self.ds_train._sampler(True) if hasattr(self.ds_train, '_sampler') else None
        if _sampler is None:
            DL = instantiate(self.dataloader.train, dataset=self.ds_train)
        else:
            ...
            DL = instantiate(self.dataloader.train, dataset=self.ds_train, sampler=_sampler)
        return DL
    
    def val_dataloader(self):
        if hasattr(self, 'ds_valid') and len(self.ds_valid) > 0:
            _sampler = self.ds_valid._sampler(False) if hasattr(self.ds_valid, '_sampler') else None
            if _sampler is None:
                DL = instantiate(self.dataloader.valid, dataset=self.ds_valid)
            else:
                for k in ['shuffle', 'sampler']:
                ...
                DL = instantiate(self.dataloader.valid, dataset=self.ds_valid, sampler=_sampler)
            return DL
        else:
            return None
    ...
    def test_dataloader(self):
        if hasattr(self, 'ds_test') and len(self.ds_test) > 0:
            _sampler = self.ds_test._sampler(False) if hasattr(self.ds_test, '_sampler') else None
            if _sampler is None:
                DL = instantiate(self.dataloader.test, dataset=self.ds_test)
            else:
                for k in ['shuffle', 'sampler']:
                ...
                DL = instantiate(self.dataloader.test, dataset=self.ds_test, sampler=_sampler)
            return DL
        else:
            return None